...
document.querySelectorAll(".style-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    const preset = STYLE_PRESETS[btn.dataset.style];
    if (!preset) return;
    document.querySelectorAll(".style-btn").forEach((b) => b.classList.toggle("active", b === btn));
    simplifySlider.value = preset.simplify;
...
// 加工已经丢失的信息了。
function enhanceImageData(imageData, structure, vivid, contrast = 0) {
  ...
  // 明度拉伸（auto-levels）。很多画作的实际明度范围是被压扁的 —— 莫奈那张
  // 只占 L* 25–76，最亮的云和最暗的伞面都远没到两端。缩到几千格之后这段
  // 本来就窄的范围还要再被量化，结果就是一片中间调的灰。
  //
  // 按分位点拉伸（而不是按最值）：一两颗噪点像素不该决定整张图的映射。
  // 拉的是亮度，不动色相 —— 所以天空会变成"干净的浅蓝 + 纯白云"，而不是
  // 整体泛白。
  if (contrast > 0.001) {
    const lum = new Float32Array(W * H);
    for (let p = 0; p < W * H; p++) {
    ...
    if (hi - lo > 1) {
      ...
      for (let p = 0; p < W * H; p++) {
        const o = p * 4;
        const y = lum[p];
        const stretched = ((y - lo) / (hi - lo)) * 255;
        // 只按 contrast 的比例往拉伸后的值靠，0 = 完全不动，1 = 拉满
        const target = y + (clamp(stretched, 0, 255) - y) * contrast;
        // 各通道按同一个比例缩放，保持色相与相对彩度不变
...
function buildReducedPalette(imageData, k) {
  ...
  for (const [key, count] of hist) {
    ...
    raw.push({ lab: rgbToLab(r, g, b), count });
  }

  // 整幅画的加权平均色 —— 用来衡量每种颜色"有多特别"。
  let mL = 0, mA = 0, mB = 0, total = 0;
  for (const p of raw) {
    mL += p.lab[0] * p.count; mA += p.lab[1] * p.count; mB += p.lab[2] * p.count;
    total += p.count;
  }
  const mean = total ? [mL / total, mA / total, mB / total] : [50, 0, 0];

  // 权重 = sqrt(面积) × 特异度加成，两项各自解决一个问题。
  //
  // 纯按面积加权，"占地最大"的颜色会吃掉几乎所有聚类名额：大片中间调的
  // 灰绿分到十几个几乎看不出区别的簇，而面积小但一眼就看见的东西 —— 一把
  // 绿伞、裙子边缘的一圈高光 —— 连一个簇都分不到，被并进灰色。sqrt 把
  // 10000px 的区域压到只值 100px 区域的 10 倍而不是 100 倍。
  //
  // 特异度 = 离全图平均色多远。早先这里用的是彩度，但那样只顾得上鲜艳的
  // 颜色，而**白色高光的彩度接近 0** —— 一件浅色衣服上那圈让形体立起来的
  // 亮边，彩度加权完全无感，照样被并掉。改成"离平均色的距离"后，鲜艳的
  // 重点色和黑白两端的极值都能保住自己的簇。
  const pts = raw.map((p) => ({
    lab: p.lab,
    w: Math.sqrt(p.count) * (1 + Math.sqrt(labDistance(p.lab, mean)) / 14),
  }));
