...
function renderUsage({ cells }) {
  ...
  for (const [code, { hex, count }] of sorted) {
    const tr = document.createElement("tr");
    const idx = codeToIndex.get(code);
    tr.className = "usage-row" + (idx === highlightIndex ? " is-active" : "");
    tr.innerHTML = `<td>${code}</td><td><span class="swatch" style="background:${hex}"></span></td><td class="num">${count}</td>`;
    tr.addEventListener("click", () => setHighlight(idx));
    usageTableBody.appendChild(tr);
  }
  const tr = document.createElement("tr");
  tr.className = "usage-total";
  tr.innerHTML = `<td>合计</td><td>${sorted.length} 色</td><td class="num">${total}</td>`;
  usageTableBody.appendChild(tr);
}
